...
export default function SideQuestEditModal({ sideQuest, accentColor, bgColor, onSave, onClose }) {
  const [title, setTitle] = useState(sideQuest.title)
  const [tickets, setTickets] = useState(sideQuest.tickets)
  const [imageKey, setImageKey] = useState(sideQuest.imageKey ?? null)
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{ marginBottom: 22 }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <div style={{ flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 0, border: '1.5px solid #e0d4e8', borderRadius: 12, overflow: 'hidden' }}>
                <button
                  type="button"
                  onClick={() => setTickets(t => Math.max(1, t - 1))}
                  style={{ width: 36, height: 44, background: '#faf6fc', border: 'none', fontSize: 18, cursor: 'pointer', color: '#9a8fa6' }}
                >−</button>
                <span style={{ width: 32, textAlign: 'center', fontFamily: "'Space Mono', monospace", fontSize: 15, color: '#3a3340' }}>{tickets}</span>
                <button
                  type="button"
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              {currentSrc
                ? <img src={currentSrc} alt="" style={{ width: 44, height: 44, objectFit: 'contain' }} />
                : <span style={{ fontSize: 28 }}>{sideQuest.icon}</span>
              }
            </div>
            <button
              type="button"
        ...
        {/* Save */}
        <button
          onClick={() => onSave({ title: title.trim() || sideQuest.title, tickets, imageKey: imageKey ?? undefined })}
          disabled={!title.trim()}
          style={{
            width: '100%',
